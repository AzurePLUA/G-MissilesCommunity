...
function ENT:OnKeyPressed()---Makes missile controllable with wasd
...
if not self.Created then 
	...
	self.chair:SetPos(self:GetPos()+self:GetForward()*105+self:GetUp()*8)
	self.chair:SetAngles(Angle(0,0,0))
	self.chair:Spawn()
	self.chair:Activate()
	self.Created = true
	self.chair:SetTrigger(true)
	self.chair:GetPhysicsObject():SetMass(2)
	self:DeleteOnRemove(self.chair)
	constraint.Weld(self.Entity,self.chair,0,0,0,true)
	constraint.NoCollide(self,self.chair,0,0)
	-------------------------------------------------------------------------
	
end

if not self.chair:IsValid() then return end
...
if Driver:IsValid() then --- We want to check if the chair has a player in it
	if not phys:IsValid() then return end
		if Driver:IsPlayer() then
			if (Driver:KeyDown(IN_MOVERIGHT)) then
			phys:AddAngleVelocity(Vector(5,0,0))
			end
			if (Driver:KeyDown(IN_MOVELEFT)) then
			phys:AddAngleVelocity(Vector(-5,0,0))
			end
			if (Driver:KeyDown(IN_BACK)) then
			phys:AddAngleVelocity(Vector(0,-6,0))
			end
